...
try {
    ...
    // Check if fontRef is a valid p5.Font object (truthy) before using textFont
...
if (isNaN(px) || isNaN(py) || isNaN(psize) || psize <= 0) {
    ...
    if (isText) {
         ...
         if (this.font && typeof graphics.textFont === 'function') {
             graphics.textFont(this.font); // Use the p5.Font object
...
if (this.type === 'text' && (!this.content || this.content.trim() === "" || this.content.trim() === TEXT_OPTIONS[0].trim()) && !this.isGrabbed) {
   ...
   if (this.type === 'text') {
       ...
       // Pass the item's font object (or fallback string) to getTextBounds
       let textBounds = getTextBounds(this.content, effectiveTextSize, this.font || baseFont);
...
usableFonts = [
    ...
    fontVT323Regular
].filter(f => f); // Filter for truthy values (successfully loaded fonts are truthy)
...
} else if (baseFont && typeof textMeasurePG.textFont === 'function') { // Fallback to monospace string
...
if (logoImage && typeof logoImage.width === 'number' && logoImage.width > 0) {
     ...
     textAlign(LEFT, CENTER);
      ...
      if (usableFonts.length > 0) {
           textFont(usableFonts[0]); // Use the first usable font for the logo
      } else {
          textFont(baseFont);
      }
...
// reset() is called in constructor, which now assigns a random font from usableFonts

newTextShape.type = 'text'; // Explicitly set type for the new shape
...
// Font is already assigned in reset() via random selection from usableFonts
 ...
 try {
    ...
    for (let i = 0; i < placedItems.length; i++) {
        ...
        pdf.noStroke();
         ...
         // Check if itemFont is a valid p5.Font object (truthy) before using it on PDF
         if (itemFont) {
